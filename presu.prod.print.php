...
$pdf->SetTitle('REPORTE DE PRESUPUESTO DE VENTA');
...
$pdf->Cell(0, 10, "Presupuesto de Venta", 0, 1, 'C');
$pdf->Ln(5);
...
if ($opcion) {
    switch ($opcion) {
        case 1:
            $cabeceras = consultas::get_datos("SELECT * FROM v_presu_prod WHERE preprod_fecha::date BETWEEN '{$_REQUEST['vdesde']}' AND '{$_REQUEST['vhasta']}'");
            break;
        case 2:
            $cabeceras = consultas::get_datos("SELECT * FROM v_presu_prod WHERE cli_cod = {$_REQUEST['vcliente']}");
            break;
        case 3:
            $cabeceras = consultas::get_datos("SELECT * FROM v_presu_prod WHERE cod_preprod IN (SELECT cod_preprod FROM detalle_presuprod WHERE cod_produ = {$_REQUEST['vproducto']})");
            break;
        case 4:
            $cabeceras = consultas::get_datos("SELECT * FROM v_presu_prod WHERE emp_cod = {$_REQUEST['vempleado']}");
            break;
    }
} else {
    $cabeceras = consultas::get_datos("SELECT * FROM v_presu_prod WHERE cod_preprod = {$_REQUEST['vcod_preprod']}");
}
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $index => $cabecera) {
        ...
        $pdf->Cell(0, 8, "Fecha: " . date('d/m/Y', strtotime($cabecera['preprod_fecha'])), 0, 1, 'R', 1);
        $pdf->Cell(100, 8, "Vendedor: " . strtoupper($cabecera['empleado']), 0, 0, 'L', 1);
        $pdf->Cell(0, 8, "Estado: " . $cabecera['preprod_estado'], 0, 1, 'R', 1);
        $pdf->Cell(100, 8, "N° Venta: " . $cabecera['cod_preprod'], 0, 1, 'L', 1);
        $pdf->Ln(10);
        ...
        $pdf->SetFont('helvetica', '', 10);
        $detalles = consultas::get_datos("SELECT * FROM v_detalle_preprod WHERE cod_preprod=" . $cabecera['cod_preprod']);
        if (!empty($detalles)) {
            $pdf->SetFillColor(255, 255, 255);
            foreach ($detalles as $det) {
                $pdf->Cell(20, 8, $det['cod_produ'], 1, 0, 'C', 1);
                $pdf->Cell(80, 8, $det['produ_descri'], 1, 0, 'L', 1);
                $pdf->Cell(20, 8, $det['preprod_cantidad'], 1, 0, 'C', 1);
                $pdf->Cell(30, 8, number_format($det['preprod_precio'], 0, ".", "."), 1, 0, 'R', 1);
            ...
            $pdf->Cell(30, 8, number_format($cabecera['preprod_monto'], 0, ".", "."), 1, 1, 'R', 1);

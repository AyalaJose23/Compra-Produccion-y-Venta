...
$pdf->SetTitle('Pedido de Materia Prima');
...
$pdf->Cell(0, 10, "Pedido de Materia Prima", 0, 1, 'C');
$pdf->Ln(5);

// Información del cliente y detalles del pedido
$opcion = $_REQUEST['opcion'] ?? null;
$filterQuery = "WHERE ped_mp = " . $_REQUEST['vped_mp'];
switch ($opcion) {
    case 1:
        $filterQuery = "WHERE ped_fecha::date BETWEEN '" . $_REQUEST['vdesde'] . "' AND '" . $_REQUEST['vhasta'] . "'";
        break;
    case 2:
        $filterQuery = "WHERE ped_mp IN (SELECT ped_mp FROM detalle_pedmp WHERE art_cod=" . $_REQUEST['varticulo'] . ")";
        break;
    case 3:
...
$cabeceras = consultas::get_datos("SELECT * FROM v_pedido_mp $filterQuery");
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->Cell(100, 8, "N° Pedido: " . $cabecera['ped_mp'], 0, 1, 'L', 1);
        $pdf->Ln(10);
        ...
        $pdf->Cell(20, 8, "Cantidad", 1, 1, 'C', 1);

        // Datos de los productos
        $pdf->SetFont('helvetica', '', 10);
        $detalles = consultas::get_datos("SELECT * FROM v_detalle_pedmp WHERE ped_mp=" . (int)$cabecera['ped_mp']);
        ...
        if (!empty($detalles)) {
            ...
            foreach ($detalles as $det) {
                ...
                $pdf->Cell(20, 8, $det['ped_cant'], 1, 1, 'C', 1);
            }
        } else {
            $pdf->Cell(0, 8, 'No se encontraron detalles', 1, 1, 'C', 1);
        }
    }
} else {
    $pdf->Cell(0, 8, 'No se encontraron datos', 1, 1, 'C', 1);
}

// Salida al navegador
$pdf->Output('pedido_mp.pdf', 'I');
?>

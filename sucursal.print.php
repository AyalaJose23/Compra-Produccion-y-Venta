       ...
       if (!empty(isset($_REQUEST['vdesde']))&& !empty(isset($_REQUEST['vhasta']))) {
            if ($_REQUEST['opcion']=='1') {
                $sucursales = consultas::get_datos("select * from sucursal "
                        . "where id_sucursal between ".$_REQUEST['vdesde']." and "
                        .$_REQUEST['vhasta']." order by id_sucursal");
            } else {
               $sucursales = consultas::get_datos("select * from sucursal "
                        . "where suc_descri between '".$_REQUEST['vdesde']."' and '"
                        .$_REQUEST['vhasta']."' order by suc_descri");
            }
         } else {

...
if ($opcion) {
    switch ($opcion) {
        case 1:
            $cabeceras = consultas::get_datos("SELECT * FROM v_compras WHERE com_fecha::date BETWEEN '{$_REQUEST['vdesde']}' AND '{$_REQUEST['vhasta']}'");
            break;
        case 2:
            $cabeceras = consultas::get_datos("SELECT * FROM v_compras WHERE prv_cod = {$_REQUEST['vproveedor']}");
            break;
        case 3:
            $cabeceras = consultas::get_datos("SELECT * FROM v_compras WHERE com_cod IN (SELECT com_cod FROM detalle_compras WHERE art_cod = {$_REQUEST['varticulo']})");
            break;
        case 4:
            $cabeceras = consultas::get_datos("SELECT * FROM v_compras WHERE emp_cod = {$_REQUEST['vempleado']}");
            break;
    }
} else {
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $index => $cabecera) {
        ...
        $pdf->Cell(100, 8, "Proveedor: " . strtoupper($cabecera['proveedor']), 0, 0, 'L', 1);
        $pdf->Cell(0, 8, "Fecha: " . date('d/m/Y', strtotime($cabecera['com_fecha'])), 0, 1, 'R', 1);
        $pdf->Cell(100, 8, "Comprador: " . strtoupper($cabecera['empleado']), 0, 0, 'L', 1);
        $pdf->Cell(0, 8, "Estado: " . $cabecera['com_estado'], 0, 1, 'R', 1);
        $pdf->Cell(100, 8, "N° Compra: " . $cabecera['com_cod'], 0, 1, 'L', 1);
        $pdf->Ln(10);
        ...
        if (!empty($detalles)) {
            ...
            foreach ($detalles as $det) {
                ...
                $pdf->Cell(20, 8, $det['com_cant'], 1, 0, 'C', 1);
                $pdf->Cell(30, 8, number_format($det['com_precio'], 0, ".", "."), 1, 0, 'R', 1);
            ...
            $pdf->Cell(30, 8, number_format($cabecera['com_total'], 0, ".", "."), 1, 1, 'R', 1);
        } else {
...
// Salida al navegador
$pdf->Output('Factura_de_compra.pdf', 'I');
?>

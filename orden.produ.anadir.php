                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="box box-primary">
                                <div class="box-header">
                                    ...
                                    <h3 class="box-title">Agregar Orden de Producción</h3>
                                    <div class="box-tools">
                                        <a href="orden.produ.index.php" class="btn btn-primary btn-sm" data-title="Volver" rel="tooltip">
                                            <i class="fa fa-arrow-left"></i> VOLVER</a>
                                    </div>
                                </div>
                                <?php $fecha = consultas::get_datos("select current_date as ord_prod_fecha");?>
                                <form action="orden.produ_control.php" method="post" accept-charset="utf-8" class="form-horizontal">
                                    <div class="box-body ">
                                        <input type="hidden" name="accion"  value="1">
                                        <input type="hidden" name="vcod_orden" value="0"/>
                                        
                                    <div class="form-group">
                                    <label class="control-label col-lg-2 col-md-2 col-sm-2">Presupuesto:</label>
                                    <div class="col-lg-5 col-md-5 col-sm-5">
                                            <select class="form-control"   required="" name="vcod_preprod"  id="pedid" 
                                                    onchange="clientes()"
                                                    onkeyup="clientes()">  
                                                <option value="">Seleccione un presupuesto</option>
                                                <?php
                                                $pedidos = consultas::get_datos("select * from v_presu_prod"
                                                                . " where preprod_estado ='PENDIENTE' "
                                                                . "order by cod_preprod");
                                                ?> 
                                                <?php foreach ($pedidos as $ped) { ?>
                                                    <option value="<?php echo $ped['cod_preprod']; ?>">
                                                        <?php echo $ped['cod_preprod'] . " - " . $ped['preprod_fecha']; ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <label class="control-label col-lg-2 col-md-2">Fecha Inicio:</label>
...
if ((parseInt($('#pedid').val()) > 0) || ($('#pedid').val() == "") || ($('#pedid').val() !== "")) {
    $.ajax({
        type: "GET",
        url: "orden.produ.presu.php?vcod_preprod=" + $('#pedid').val(),
        cache: false,
        beforeSend: function () {

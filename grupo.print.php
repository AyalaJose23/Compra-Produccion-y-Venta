...
//COLOR DE TABLA
        ...
        if (!empty(isset($_REQUEST['vdesde']))&& !empty(isset($_REQUEST['vhasta']))) {
            if ($_REQUEST['opcion']=='1') {
                $grupos = consultas::get_datos("select * from grupos "
                        . "where gru_cod between ".$_REQUEST['vdesde']." and "
                        .$_REQUEST['vhasta']." order by gru_cod");
            } else {
               $grupos = consultas::get_datos("select * from grupos "
                        . "where gru_nombre between '".$_REQUEST['vdesde']."' and '"
                        .$_REQUEST['vhasta']."' order by gru_nombre");
            }
         } else {
        ...
        foreach ($grupos as $gru) {
            $pdf->Cell(50,5,$gru['gru_cod'], 1, 0, 'C', 1);
            $pdf->Cell(0,5,$gru['gru_nombre'], 1, 0, 'C', 1);
            $pdf->Ln();
        }

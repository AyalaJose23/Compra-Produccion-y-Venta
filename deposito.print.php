...
//COLOR DE TABLA
       ...
       // $pdf->Cell(30,5,'C.I', 1, 0, 'C', 1);
        $pdf->Cell(0,5,'DESCRIPCION', 1, 0, 'C', 1);
       // $pdf->Cell(50,5,'APELLIDO', 1, 0, 'C', 1);
       // $pdf->Cell(40,5,'TELEFONO', 1, 0, 'C', 1);
        ...
        if (!empty(isset($_REQUEST['vdesde']))&& !empty(isset($_REQUEST['vhasta']))) {
            if ($_REQUEST['opcion']=='1') {
                $depositos = consultas::get_datos("select * from v_deposito "
                        . "where dep_cod between ".$_REQUEST['vdesde']." and "
                        .$_REQUEST['vhasta']." order by dep_cod");
            } else {
                $depositos = consultas::get_datos("select * from v_deposito "
                        . "where dep_descri between '".$_REQUEST['vdesde']."' and '"
                        .$_REQUEST['vhasta']."' order by dep_descri");
            }
         } else {
        ...
        foreach ($depositos as $depo) {
            $pdf->Cell(25,5,$depo['dep_cod'], 1, 0, 'C', 1);
            //$pdf->Cell(30,5,$cli['cli_ci'], 1, 0, 'C', 1);
            $pdf->Cell(0,5,$depo['dep_descri']." ".$depo['suc_descri'], 1, 0, 'C', 1);
            //$pdf->Cell(40,5,$cli['cli_telefono'], 1, 0, 'C', 1);
           // $pdf->Cell(40,5,$cli['cli_direcc'], 1, 0, 'C', 1);
            $pdf->Ln();
        }

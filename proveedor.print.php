...
//COLOR DE TABLA
        ...
        if (!empty(isset($_REQUEST['vdesde']))&& !empty(isset($_REQUEST['vhasta']))) {
            if ($_REQUEST['opcion']=='1') {
                $proveedores = consultas::get_datos("select * from proveedor "
                        . "where prv_cod between ".$_REQUEST['vdesde']." and "
                        .$_REQUEST['vhasta']." order by prv_cod");
            } else {
                $proveedores = consultas::get_datos("select * from proveedor "
                        . "where prv_ruc between '".$_REQUEST['vdesde']."' and '"
                        .$_REQUEST['vhasta']."' order by prv_ruc");
            }
         } else {
        ...
        foreach ($proveedores as $provee) {
            $pdf->Cell(25,5,$provee['prv_cod'], 1, 0, 'C', 1);
            $pdf->Cell(30,5,$provee['prv_ruc'], 1, 0, 'C', 1);
            $pdf->Cell(50,5,$provee['prv_razonsocial'], 1, 0, 'C', 1);
            $pdf->Cell(40,5,$provee['prv_direccion'], 1, 0, 'C', 1);
            $pdf->Cell(40,5,$provee['prv_telefono'], 1, 0, 'C', 1);
            $pdf->Ln();
        }
